...
- Код следует стандартам проекта (Effective Dart, Flutter Style Guide)?
...
**Пример проблемы:**
```
❌ Добавлен файл discount_service.dart, но он не упомянут в src/services/.AGENTS.md
...
🟢 Рекомендация: Можно упростить проверку user_level
   - Файл: src/services/discount_service.dart, строки 23-30
   - Рекомендация: Вместо цепочки if-else можно использовать Map с методом discount_rates[user_level] ?? 0.0
```
