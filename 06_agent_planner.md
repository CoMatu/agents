...
### Новые файлы
- `path/to/new_file.dart` — [назначение файла]

### Изменения в существующих файлах

#### Файл: `path/to/existing_file.dart`

**Класс `ClassName`:**
- Добавить метод `methodName({required Type1 param1,required Type2 param2}) { return ReturnType; }`
  - Параметры:
    - `param1` — [описание]
    - `param2` — [описание]
  - Возвращает: [описание]
...
**Метод `methodName`:**
- Добавить параметр `Type newParam` — [описание]
...
- Изменить метод `bool processPayment(double amount) return result;`
  - Добавить параметр `String currency = "USD"`
...
❌ **НЕ пиши тесты для виджетов** — не пиши тесты для виджетов, только юнит-тесты
